...
Flat-output benchmark runner for the single-feature pulse-excitation
experiments.
...
FEATURE_GROUPS = {
    "Hyst": [
        "Hyst_M3_0.5C", "Hyst_M3_1C", "Hyst_M3_1.5C", "Hyst_M3_2C", "Hyst_M3_2.5C",
    ],
    "R0": [
        "R0_p_0.5C", "R0_n_0.5C",
        "R0_p_1C", "R0_n_1C",
...
DEFAULT_GRID_FAMILY_ALIASES = {
    # Experiment naming has moved to "fai_irrev", while the current raw Excel
    # exports still store the corresponding feature family under Hyst_M3_*C.
    "fai_irrev": ["fai_irrev", "phi_irrev", "faiirrev", "phiirrev", "irrev", "hyst_m3", "hystm3"],
}
...
def canonicalize_grid_feature_name(column_name: Any, feature_family: str) -> str:
    ...
    if family == "fai_irrev":
        return re.sub(r"(?i)hyst[_ ]?m3", "fai_irrev", name)
    return name
...
# Public single-feature experiments report the diffusion-scaled irreversible
# feature family as "fai_irrev". The current feature-table exports keep the
# corresponding raw columns under "Hyst_M3_*C", so rewrite output labels after
# the core benchmark has finished.
_CORE_RUN = run


def _public_text(value):
    if isinstance(value, str):
        return value.replace("Hyst_M3", "fai_irrev").replace("hyst_m3", "fai_irrev")
...
def _rewrite_outputs_to_public_names(run_dir):
    run_dir = Path(run_dir)

    for name in ["config.json", "Exp_summary.json", "case_status.json"]:
        path = run_dir / name
        if not path.exists():
            continue
        payload = json.loads(path.read_text(encoding="utf-8"))
        path.write_text(json.dumps(_public_text(payload), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    for name in [
        ...
        for column in object_cols:
            frame[column] = frame[column].map(_public_text)
        frame.to_csv(path, index=False)


def run(cfg: Dict[str, Any]) -> Path:
    run_dir = _CORE_RUN(cfg)
    _rewrite_outputs_to_public_names(run_dir)
    return run_dir
