...
# Public single-feature experiments report the diffusion-scaled pulse feature
# family as "fai_irrev". Current feature tables use Fai_*C as the raw column
# label, while older internal exports used Hyst_M3_*C, so rewrite output labels
# after the core benchmark has finished.
_CORE_RUN = run


def _public_text(value):
    if isinstance(value, str):
        value = value.replace("Hyst_M3", "fai_irrev").replace("hyst_m3", "fai_irrev")
        return re.sub(r"(?i)\\bFai(?=_)", "fai_irrev", value)
