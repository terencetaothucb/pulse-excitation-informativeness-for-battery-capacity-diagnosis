...
- features: Fai_*C raw columns, reported as fai_irrev_*C
...
DEFAULT_RAW_FEATURE_REGEX = r"^Fai_"
...
def _public_feature_name(feature: str) -> str:
    name = str(feature)
    name = re.sub(r"(?i)^hyst[_ ]?m3(?=_)", "fai_irrev", name)
    name = re.sub(r"(?i)^fai(?=_)", "fai_irrev", name)
    return name


def _raw_feature_name(feature: str) -> str:
    name = str(feature)
    name = re.sub(r"(?i)^hyst[_ ]?m3(?=_)", "Fai", name)
    name = re.sub(r"(?i)^fai_irrev(?=_)", "Fai", name)
    name = re.sub(r"(?i)^phi_irrev(?=_)", "Fai", name)
    return name
...
def _discover_features(data_root: Path, materials: Sequence[str], widths: Sequence[int]) -> List[str]:
    ...
    if explicit:
        return [_raw_feature_name(feature) for feature in explicit]
    ...
    if not features:
        raise RuntimeError(
            "No Fai/fai_irrev features discovered from {} sheet {}. Check FEATURES or FEATURE_REGEX.".format(xlsx_path, sheet_name)
        )
    return features
