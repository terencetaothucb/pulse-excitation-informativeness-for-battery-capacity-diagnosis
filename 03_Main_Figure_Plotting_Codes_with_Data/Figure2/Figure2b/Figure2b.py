...
"""
Figure 2b for PulseBat-combo.

68Ah LFP, SOC=50%, pulse width=5000 ms, single-pulse features.
...
SOH_CMAP_COLORS = ("#D82E45", "#E98667", "#E9A08D", "#83B5B8", "#DAA250")
SOH_CMAP = mpl.colors.LinearSegmentedColormap.from_list(
    "figure2b_soh_red_peach_cyan_yellow",
    SOH_CMAP_COLORS,
)
...
def write_points_csv(records: Sequence[PredictionRecord], out_dir: Path) -> None:
    out_csv = out_dir / "Figure2b_linear_test_points.csv"
...
def write_panel_annotation_csv(records: Sequence[PredictionRecord], out_dir: Path) -> None:
    path = out_dir / "Figure2b_panel_annotations.csv"
...
def error_bins(records: Sequence[PredictionRecord]) -> np.ndarray:
    return np.arange(-0.07, 0.0701, 0.01)
...
def plot(records: Sequence[PredictionRecord], out_dir: Path, fig_w_cm: float, fig_h_cm: float, dpi: int) -> None:
    ...
    png_path = out_dir / "Figure2b.png"
    pdf_path = out_dir / "Figure2b.pdf"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Draw PulseBat-combo Figure2b.")
    ...
    if results_root.exists():
        ...
        cached_csv = out_dir / "Figure2b_linear_test_points.csv"
