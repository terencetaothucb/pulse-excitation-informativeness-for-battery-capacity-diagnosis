...
"""
Figure 2c for PulseBat-combo.

20Ah LFP single-pulse linear-model MAE versus SOC.
...
def write_mae_csv(records: Sequence[MaeRecord], out_dir: Path) -> None:
    out_csv = out_dir / "Figure2c_linear_mae_vs_soc.csv"
...
def plot(records: Sequence[MaeRecord], out_dir: Path, fig_w_cm: float, fig_h_cm: float, dpi: int) -> None:
    ...
    png_path = out_dir / "Figure2c.png"
    pdf_path = out_dir / "Figure2c.pdf"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Draw PulseBat-combo Figure2c.")
    ...
    if results_root.exists():
        ...
        records = load_cached_records(out_dir / "Figure2c_linear_mae_vs_soc.csv")
