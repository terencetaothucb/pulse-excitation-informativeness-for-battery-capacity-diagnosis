...
"""
Figure 5g for PulseBat-combo.

Integrated model ranking using the original metric weights:
Accuracy 40, Stability 20, Inference 20, Fit 10, normalized by 90.
"""
...
def plot(rows: Sequence[Dict[str, object]], out_dir: Path, fig_w_cm: float, fig_h_cm: float, dpi: int) -> None:
    ...
    write_csv(out_dir / "Figure5g_integrated_ranking_original_weights.csv", rows)
    ...
    ax_heat = fig.add_axes([0.1943, 0.2990, 0.2957, 0.5810])
    cax = fig.add_axes([0.1943, 0.070, 0.2957, 0.026])
    ax_bar = fig.add_axes([0.625, 0.274, 0.350, 0.606])
    ...
    for row_idx in range(score_values.shape[0]):
        for col_idx in range(score_values.shape[1]):
            ...
            ax_heat.text(col_idx, row_idx, f"{value:.0f}", ha="center", va="center", fontsize=7, color="white" if value > 55 else "black")
    ...
    fig.text(0.50, 0.985, TITLE, ha="center", va="top", fontsize=7)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Draw PulseBat-combo Figure5g.")
